...
class HomeViewModel {
  ...
  func loadAllGames() {
    ...
    var data: [String: Any?] = [:]
    if let genre = self.selectedGenreResult {
      data["genres"] = genre.id

      self.selectedGenreTitle = genre.name ?? ""
    } else {
      self.selectedGenreTitle = "All Games"
    }

    dataGame = nil

    let withGenre = self.selectedGenreResult == nil
    gamesUseCase.get().loadAllGames(param: data, withGenres: withGenre)
      .receive(on: RunLoop.main)
      .sink { [weak self] completion in
        switch completion {
        case .failure(let error): self?.isFailed.send(error.localizedDescription)
        case .finished: break
        }
