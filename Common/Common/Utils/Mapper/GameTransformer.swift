...
public struct GameTransformer: Mapper {
  ...
  public func transformResponseToDomain(response: [GameResultResponse]) -> [GameModel] {
    return response.map { data in
      return transformResponseToDomain(response: data)
    }
  }
  ...
  public func transformResponseToEntity(response: [GameResultResponse]) -> Any {
    fatalError()
  }
  ...
  public func transformDomainToEntity(domain: [GameModel]?) -> Any {
    fatalError()
  }
}
